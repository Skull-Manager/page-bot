...
**Там уже будет папка `public_html`, откройте `скачанный архив`**   
...
**На спринхосте вы копируете ваш `url-адрес сайта`, вставляете в строку, переходите, там будет `hello skull`. В строке браузера к ссылке вашего сайта прибавляете `/page-bot-master/bot.php` - это и есть нужный адрес для бота.**


# У меня не получается установить бота
Создайте на сервере в папке `public_html` файл `skull.php` и вставьте этот код в пустой файл ->
...
```
После чего в браузере открой свой сайт и в строку адресса добавьте `/skull.php`, после чего установится все на сервер
