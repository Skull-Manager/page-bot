...
include 'custom_code.php'; // произвольный (пользовательский) код (свой код пишите в этом файле (custom_code) и больше нигде, т.к при обновлении ваш код пропадет (если будете изменять прямо здесь) )
    
/* зарезервированные переменные (которые не стоит объявлять повторно)
$peer_id      - id чата
